	...
	<div class="CatalogBlock">
		<div class="CatalogList">
			<div style="display: flow-root;">
				...
				<div class="CatalogListButton" onclick="setSeasonStyle()" id="SeasonGrid_Small" style="display:none;">
...
function getSeasonStyle(){
	...
	if(styleSeason == '0'){
		...
		document.getElementById('SeasonGrid_Small').style.display = "none";
		document.getElementById('SeasonList').style.display = "block";
	} else if(styleSeason == '2'){
		document.getElementById('SeasonGrid').style.display = "none";
		document.getElementById('SeasonGrid_Small').style.display = "block";
...
function setSeasonStyle(){
	if(styleSeason == 1){
		...
		document.getElementById('SeasonGrid').style.display = "none";
		document.getElementById('SeasonList').style.display = "none";
		document.getElementById('SeasonGrid_Small').style.display = "";
	} else if(styleSeason == 2){
		styleSeason = 0;
		localStorage.setItem('styleCatalog', '0');
		document.getElementById('SeasonGrid').style.display = "none";
...
function GeneratorSeason(n) {
	if(n == 1) {
		numAft = 0
	} else {
		numAft = after
	}
	for (let i = numAft; SeasonList.length > i; i++) {
		...
		if(SeasonList[i].player.series.last != null){
			...
			var seriesSH = "Нэма...";
			if(Object.keys(SeasonList[i].player).length != 0){
				seriesSH = `${SerieType} ${SeasonList[i].player.series.last}${SerieLength}`;
			}
		}
		...
		if(styleSeason == 0){
			...
			div.innerHTML += `
				<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M464 64H48C21.49 64 0 85.49 0 112v288c0 26.51 21.49 48 48 48h416c26.51 0 48-21.49 48-48V112c0-26.51-21.49-48-48-48zm-6 336H54a6 6 0 0 1-6-6V118a6 6 0 0 1 6-6h404a6 6 0 0 1 6 6v276a6 6 0 0 1-6 6zM128 152c-22.091 0-40 17.909-40 40s17.909 40 40 40 40-17.909 40-40-17.909-40-40-40zM96 352h320v-80l-87.515-87.515c-4.686-4.686-12.284-4.686-16.971 0L192 304l-39.515-39.515c-4.686-4.686-12.284-4.686-16.971 0L96 304v48z"/></svg>
				${stylePoster}
				<a class="LineCard-Hover" onclick="goRoute('/release', {id:${SeasonList[i].id}})">
