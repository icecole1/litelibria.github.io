...
function LoadApiSearch(s1) {
	...
	// Запрос к Api 
	var url = config["titels_api"]+"searchTitles?search="+s1+"&filter=id,names.ru,posters.medium,player.series,description,genres&limit="+num+"&after="+after;
  fetch(url)
...
function GeneratorSearch() {
	for (let i = 0; SearchList.length > i; i++) {
		...
		if(SearchList[i].player.series.last != null){
			TextSerie = `<div class="LineCard-TextSerie">Серия ${SearchList[i].player.series.last}</div>`;
		}
